...
export const COPY = {
  ...
  modeStep: {
    title: '先选一条路线',
    lead: '两条路线共用同一个后端，测的是同一批东西，只是精度不同。',
    nocard: {
      badge: '日常够用',
      label: '直接拍一张',
      sub: '不用打印任何东西',
      desc: '适合大多数人。光线好的时候深浅很准；冷暖只作参考，测不准会直接告诉你。',
    },
    card: {
      badge: '精度更高',
      label: '对着参考卡拍',
      sub: '需要先打印一张 A4 参考卡',
      desc: '把参考卡放在脸旁边一起拍，颜色就有了标尺，冷暖也敢下结论。',
    },
    hintCard: '先打印参考卡。拍的时候把它平放在脸旁、同一个光线下，让四个黑色方框完整入镜。',
    hintNoCard: '不用任何道具。测不准的时候我们会直接说，并告诉你怎么重拍。',
    cardProfileLabel: '参考卡编号（可选）',
    cardProfilePlaceholder: 'my-card-001',
    linksLabel: '没标定过就留空。',
    cardLink: '打印参考卡',
    calibrateLink: '先标定我的卡',
    illuminantLabel: '这张照片是在什么光下拍的？',
    finePrint: '会测三件事：皮肤有多深、底色偏冷还是偏暖、表面的油光和泛红。三者互不影响。',
  },
  ...
  captureStep: {
    ...
    chooseFile: '选择图片',
    guideTitle: '怎么拍才测得准',
    guideLead: '最关键的一条：站到窗边，让自然光照在脸上。',
    guideItems: [
      '站到窗边、脸朝窗户。别站在顶灯正下方——顶灯和窗光混在一起，算法最难校正。',
      '关掉美颜、滤镜、HDR。它们会直接改掉皮肤的颜色。',
      '让脸和脖子都完整入镜。别逆光，窗户不要在身后。',
      '素颜最好；至少下颌和脖子别带妆。刚运动完或刚洗完脸，等十分钟再拍。',
      '摘掉眼镜，头发别挡住下颌线。',
    ],
    cameraInfo: (w, h) => (w && h ? `${w}×${h}` : ''),
    cameraUnsupported: '这个浏览器不给摄像头权限（file:// 下常见）。用手动选图吧。',
    shutterTooEarly: '摄像头还没有出画，等一秒再按快门。',
    capturedToast: '已拍下。可以直接分析；想更准也可以框选皮肤区域。',
    closedCameraToast: '已关闭摄像头。',
    fileReadToast: '图片已读入。可以直接分析，也可以框选皮肤区域。',
  ...
  /* ───────── 上传同意 ─────────
     实质内容（去哪了 / 留多久 / 怎么删 / 指纹 / 第三方）全部写在 privacy.html，
     界面上只留一行"一键同意"——用户不需要在上传前读一段技术说明。 */
  consent: {
